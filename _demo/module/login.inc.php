...
function login()
{
    ...
    if ($_POST)
    {
        if (empty($_POST['username']) || empty($_POST['password']))
            json_return(null, 1, __('Username and password cannot be empty.'));

        if (str::check($_POST['username'], 'email')) $key = 'email';
        else if (str::check($_POST['username'], '/^1[0-9]{10}$/')) $key = 'tel';
        else $key = 'username';

        $user = $db -> prepare("SELECT `id`,`name`,`password`,`md` FROM `rbac_user` WHERE BINARY `{$key}`=:user") -> execute(array(':user'=>$_POST['username']));

        if (!$user || $user[0]['password'] != md5(md5($_POST['password']).$user[0]['md']))
        {
            json_return(null, 1, __('Username or password is incorrect.'));
        }
        else
        {
