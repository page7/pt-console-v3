...
$.fn.zdatepicker = function(options) {
	...
	var checkArea = function(start, end) {
		for(var i = start.getTime(); i < end.getTime(); i = i+24*3600*1000) { // last date not check in hotel
			var temp = new Date();
			temp.setTime(i);
	...
	var buildCalendar = function(obj) {
		...
		input.bind(opts.event, function(){
			...
			if(date.length && !opts.selected){
				lastSelected = date.join("-");
				initSelect(opts.selected ? $.extend( {0:{0:lastSelected}}, opts.selected ) : {0:{0:lastSelected}});
	...
	var _dateList = function(year, month, days) {
		...
		for(var i=1; i<=days; i++) {
			...
			if(isArea(year, month, i)) {
                cla.push('area');
                if (!areas) {
                    cla.push('areaStart');
                    areas = true;
                }
                if (i==days || !isArea(year, month, i+1)) {
                    cla.push('areaEnd');
                }
            }
	...
	var changeArea = function(type, newArea, date, dateObj, obj, calendar, a, selected) {
		...
		if((type == 'end' && newArea[0] == '') || (type == 'start' && newArea[1] == '')) {
			...
			$.fn.zdatepicker.callback("area", {date:date, dateobj:dateObj, input:obj, calendar:calendar, a:a, area:[getDateObj(newArea[0]), getDateObj(newArea[1])]});
		...
		if(!isArea(dateObj.getFullYear(), dateObj.getMonth()+1, dateObj.getDate(), newArea)) {
			...
			calendar.find(".area").removeClass("area");
			// callback
			$.fn.zdatepicker.callback("fail_to_area", {date:date, dateobj:dateObj, input:obj, calendar:calendar, a:a, area:[getDateObj(newArea[0]), getDateObj(newArea[1])]});
			return;
            ...
            days.children().removeClass("area areaStart areaEnd");
			days.slice(start, end+1).children("a,span").addClass("area");
			days.eq(start).children("a,span").addClass("areaStart");
			days.eq(end).children("a,span").addClass("areaEnd");
			a.addClass("selected");
